...
trait CacheableEloquent
{
    ...
    public function cacheQuery($builder, array $columns, Closure $closure)
    {
        ...
        if (method_exists(app('cache')->getStore(), 'tags')) {
            $result = $lifetime === -1
                ? app('cache')->tags($modelName)->rememberForever($cacheKey, $closure)
                : app('cache')->tags($modelName)->remember($cacheKey, $lifetime, $closure);

            return $result;
        }

        $result = $lifetime === -1
            ? app('cache')->rememberForever($cacheKey, $closure)
            : app('cache')->remember($cacheKey, $lifetime, $closure);
